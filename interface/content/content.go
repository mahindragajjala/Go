...
- `"{"` and `"}"` are curly braces — the body of the interface.
- `{ InterfaceElem ";" }` means:
    Inside the braces, you can write **zero or more `InterfaceElem`** (elements) — each separated by a semicolon (`;`).
...
This can hold **any value** — because it has no method requirements.
...
- Each method can end with a semicolon (`;`) — Go doesn't force you to write `;` in code, but the compiler sees it.
